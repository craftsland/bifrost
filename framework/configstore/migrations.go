...
func migrationMergeOauthTokenTables(ctx context.Context, db *gorm.DB, logger schemas.Logger) error {
	...
	return RunSingleMigration(ctx, nil, db, logger, &migrator.Migration{
		...
		Rollback: func(tx *gorm.DB) error {
			...
			// mcp_oauth_tokens is wholly new as of this migration, so rolling
			// back simply drops it — oauth_tokens and oauth_user_tokens
			// were never written to by Migrate above and need no repair.
			if mg.HasTable(&tables.TableMCPOauthToken{}) {
				logger.Info("[configstore] %s: dropping table TableMCPOauthToken", migrationName)
				if err := mg.DropTable(&tables.TableMCPOauthToken{}); err != nil {
					return fmt.Errorf("drop mcp_oauth_tokens table: %w", err)
				}
			}

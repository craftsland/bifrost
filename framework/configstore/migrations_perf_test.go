...
// shared-credential table) via batched multi-row INSERTs wrapped in a single
// transaction — not one .Create() call per row — so seeding time stays a
// small fraction of the migration time being measured.
func bulkInsertOauthTokens(t *testing.T, db *gorm.DB, count int) {
	t.Helper()
	now := time.Now()
	expiresAt := now.Add(24 * time.Hour)
	batch := make([]tables.TableOauthToken, 0, perfSeedInsertBatchSize)
	require.NoError(t, db.Transaction(func(tx *gorm.DB) error {
		for i := 0; i < count; i++ {
			batch = append(batch, tables.TableOauthToken{
				ID:               fmt.Sprintf("perf-shared-tok-%d", i),

...
func (s *RDBConfigStore) DeleteSharedOauthTokensByConfigID(ctx context.Context, oauthConfigID string) error {
	if oauthConfigID == "" {
		return nil
	}
	result := s.DB().WithContext(ctx).
...
//
// Wrapped in a transaction so SELECT + CREATE/UPDATE is atomic under
// concurrent same-binding races.
...
func (s *RDBConfigStore) claimOauthFlowByStateAndModes(ctx context.Context, state string, modes []string) (*tables.TableMCPOauthFlow, error) {
	var flow tables.TableMCPOauthFlow
	result := s.DB().WithContext(ctx).Where("state = ? AND status = ? AND flow_mode IN ?", state, "pending", modes).First(&flow)
	if result.Error != nil {
		if errors.Is(result.Error, gorm.ErrRecordNotFound) {
			return nil, nil
		}
		return nil, fmt.Errorf("failed to claim oauth flow by state: %w", result.Error)
	}

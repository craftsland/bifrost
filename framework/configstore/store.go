...
type ConfigStore interface {
	...
	// any existing row for the same binding (mcp_client_id for shared;
	// identity column + mcp_client_id for per-identity modes) and reusing
	// its ID, matching this table's partial unique indexes. Was two separate
	...
	DeleteSharedOauthTokensByConfigID(ctx context.Context, oauthConfigID string) error

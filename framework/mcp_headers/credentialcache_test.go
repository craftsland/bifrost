...
func TestHeaderCredentialCache_HitAndMiss(t *testing.T) {
	...
	v, err := c.Fill("k1", fillWithCredential(testCredential("c1", map[string]string{"X-Api-Key": "v1"})))
...
func TestHeaderCredentialCache_EvictExactKey(t *testing.T) {
	c := newHeaderCredentialCache(4)
	_, err := c.Fill("k1", fillWithCredential(testCredential("c1", nil)))
	require.NoError(t, err)
...
func TestHeaderCredentialCache_EvictByCredentialID(t *testing.T) {
	...
	_, err := c.Fill("k1", fillWithCredential(testCredential("c1", nil)))
	require.NoError(t, err)
	_, err = c.Fill("k2", fillWithCredential(testCredential("c2", nil)))
	require.NoError(t, err)
...
func TestHeaderCredentialCache_EvictByMCPClient(t *testing.T) {
	...
	for i, k := range []string{keyA1, keyA2, keyA3, keyB} {
		_, err := c.Fill(k, fillWithCredential(testCredential(fmt.Sprintf("c%d", i), nil)))
		require.NoError(t, err)
	}
...
func TestHeaderCredentialCache_EvictByVirtualKey(t *testing.T) {
	...
	for i, k := range []string{keyVK1A, keyVK1B, keyVK2, keyUser} {
		_, err := c.Fill(k, fillWithCredential(testCredential(fmt.Sprintf("vc%d", i), nil)))
		require.NoError(t, err)
	}
...
func TestHeaderCredentialCache_EvictByUser(t *testing.T) {
	...
	for i, k := range []string{keyU1A, keyU1B, keyU2, keyVK} {
		_, err := c.Fill(k, fillWithCredential(testCredential(fmt.Sprintf("uc%d", i), nil)))
		require.NoError(t, err)
	}
...
func TestHeaderCredentialCache_Flush(t *testing.T) {
	c := newHeaderCredentialCache(4)
	for i := 1; i <= 3; i++ {
		_, err := c.Fill(fmt.Sprintf("k%d", i), fillWithCredential(testCredential(fmt.Sprintf("c%d", i), nil)))
		require.NoError(t, err)
	}
...
func TestHeaderCredentialCache_InflightDedup(t *testing.T) {
	...
	go func() {
		...
		v, err := c.Fill("k1", func() (cachedHeaderCredential, error) {
			calls.Add(1)
...
func TestHeaderCredentialCache_ErrorSharedNotCached(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		_, err := c.Fill("k1", func() (cachedHeaderCredential, error) {
			calls.Add(1)
			close(started)
			<-release
...
func TestHeaderCredentialCache_UpsertRebindsCredentialIDIndex(t *testing.T) {
	c := newHeaderCredentialCache(4)
	_, err := c.Fill("k1", fillWithCredential(testCredential("c-old", nil)))
	require.NoError(t, err)
	...
	c.Evict("k1")
	_, err = c.Fill("k1", fillWithCredential(testCredential("c-new", nil)))
	require.NoError(t, err)
...
func TestHeaderCredentialCache_NilSafety(t *testing.T) {
	...
	c.Flush()
	assert.Equal(t, 0, c.Len())
	v, err := c.Fill("k", fillWithCredential(testCredential("c-pass", nil)))
	require.NoError(t, err)

...
func TestUserTokenCache_HitAndMiss(t *testing.T) {
	...
	v, err := c.Fill("k1", fillWith(testToken("t1", "access-1", nil)))
...
func TestUserTokenCache_ExpiredEntryIsMissAndRemoved(t *testing.T) {
	...
	_, err := c.Fill("k1", fillWith(testToken("t1", "stale-access", &past)))
	require.NoError(t, err)
...
func TestUserTokenCache_EvictExactKey(t *testing.T) {
	c := newUserTokenCache(4)
	_, err := c.Fill("k1", fillWith(testToken("t1", "access-1", nil)))
	require.NoError(t, err)
...
func TestUserTokenCache_EvictByTokenID(t *testing.T) {
	...
	_, err := c.Fill("k1", fillWith(testToken("t1", "access-1", nil)))
	require.NoError(t, err)
	_, err = c.Fill("k2", fillWith(testToken("t2", "access-2", nil)))
	require.NoError(t, err)
...
func TestUserTokenCache_EvictByMCPClient(t *testing.T) {
	...
	for i, k := range []string{keyA1, keyA2, keyB} {
		_, err := c.Fill(k, fillWith(testToken(fmt.Sprintf("t%d", i), "access", nil)))
		require.NoError(t, err)
	}
...
func TestUserTokenCache_EvictByVirtualKey(t *testing.T) {
	...
	for i, k := range []string{keyVK1A, keyVK1B, keyVK2, keyUser} {
		_, err := c.Fill(k, fillWith(testToken(fmt.Sprintf("vt%d", i), "access", nil)))
		require.NoError(t, err)
	}
...
func TestUserTokenCache_EvictByUser(t *testing.T) {
	...
	for i, k := range []string{keyU1A, keyU1B, keyU2, keyVK} {
		_, err := c.Fill(k, fillWith(testToken(fmt.Sprintf("ut%d", i), "access", nil)))
		require.NoError(t, err)
	}
...
func TestUserTokenCache_Flush(t *testing.T) {
	c := newUserTokenCache(4)
	for i := 1; i <= 3; i++ {
		_, err := c.Fill(fmt.Sprintf("k%d", i), fillWith(testToken(fmt.Sprintf("t%d", i), "access", nil)))
		require.NoError(t, err)
	}
...
func TestUserTokenCache_InflightDedup(t *testing.T) {
	...
	go func() {
		...
		v, err := c.Fill("k1", func() (cachedUserToken, error) {
			calls.Add(1)
...
func TestUserTokenCache_ErrorSharedNotCached(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		_, err := c.Fill("k1", func() (cachedUserToken, error) {
			calls.Add(1)
			close(started)
			<-release
...
func TestUserTokenCache_UpsertRebindsTokenIDIndex(t *testing.T) {
	c := newUserTokenCache(4)
	_, err := c.Fill("k1", fillWith(testToken("t-old", "old", nil)))
	require.NoError(t, err)
	...
	c.Evict("k1")
	_, err = c.Fill("k1", fillWith(testToken("t-new", "new", nil)))
	require.NoError(t, err)
...
func TestUserTokenCache_NilSafety(t *testing.T) {
	...
	c.Flush()
	assert.Equal(t, 0, c.Len())
	v, err := c.Fill("k", fillWith(testToken("t", "pass-through", nil)))
	require.NoError(t, err)

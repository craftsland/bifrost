...
// TestWarmSemanticExemplarsReusesMatchingMarker avoids provider calls on a
// warm process when the VectorStore already holds the exact exemplar generation.
...
func TestWarmSemanticExemplarsUsesBackendCompatibleLifecycle(t *testing.T) {
	...
	assert.Equal(t, semanticGenerationNamespace(semanticFingerprint(&config, semanticExemplars(&config))), namespace)
...
func TestSemanticClassifierSwitchesGenerationsOnlyAfterSuccessfulWarmup(t *testing.T) {
	...
	// F1 remains present for another replica that has not switched yet.
	v1Fingerprint := semanticFingerprint(&v1, semanticExemplars(&v1))
	v1Markers, err := activeStore.GetChunks(context.Background(), v1Namespace, []string{semanticMarkerID(v1Fingerprint)})
	require.NoError(t, err)
	require.Len(t, v1Markers, 1)
	...
	v3Fingerprint := semanticFingerprint(&v3, semanticExemplars(&v3))
	v3Markers, err := activeStore.GetChunks(
		context.Background(),
		semanticGenerationNamespace(v3Fingerprint),
		[]string{semanticMarkerID(v3Fingerprint)},
	)
	require.NoError(t, err)
	assert.Empty(t, v3Markers, "a failed generation must not receive its completion marker")
}

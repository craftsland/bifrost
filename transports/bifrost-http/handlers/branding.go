...
// RegisterRoutes wires the branding read endpoints onto r.
//
// Both routes are registered without the auth middleware chain: the login
// screen renders the logo before any session exists, so gating these behind
// auth would guarantee the default Bifrost logo flashes on every login. The
// exposure is limited to two images an operator deliberately uploaded for
// display on that very screen.
func (h *BrandingHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.BifrostHTTPMiddleware) {
	...
	r.GET("/api/branding/asset/{kind}", h.getAsset)
}

...
func (s *BifrostHTTPServer) RegisterAPIRoutes(ctx context.Context, callbacks ServerCallbacks, middlewares ...schemas.BifrostHTTPMiddleware) error {
	...
	// Registers its own read routes without the auth chain — the login screen
	// needs the logo before a session exists.
	brandingHandler.RegisterRoutes(s.Router, middlewares...)

...
const analyzerConfigSchema = z
	...
	.superRefine((data, ctx) => {
		// The phrase caps and the one-tier-per-phrase rule only apply when the lists
		// are also used as semantic exemplars. Mirrors validateComplexitySemanticPhrases
		// so an over-budget list fails in the form instead of as an opaque 400.
		...
		const total = lists.reduce((sum, { key }) => sum + data.keywords[key].length, 0);
		if (total > MAX_SEMANTIC_PHRASES) {
			ctx.addIssue({
				code: "custom",
				message: `Semantic mode supports at most ${MAX_SEMANTIC_PHRASES} phrases across all three tiers (currently ${total}).`,
				path: ["keywords"],
			});
		}
											...
											<Controller
												...
												render={({ field }) => (
													<Input
														...
														className={cn(
															"font-mono",
															semanticErrors?.min_similarity && "border-destructive focus-visible:ring-destructive",
														)}
													/>
												)}
											/>
						...
						{isSemantic && (
							<span
								className={cn(
									...
									totalPhrases > MAX_SEMANTIC_PHRASES ? "text-destructive" : "text-muted-foreground",
								)}
								data-testid="complexity-router-phrase-total"
							>
								{totalPhrases}/{MAX_SEMANTIC_PHRASES} phrases
							</span>
						)}
					</div>

					{/* Root-level keyword issues (total cap, cross-tier duplicates) have no
